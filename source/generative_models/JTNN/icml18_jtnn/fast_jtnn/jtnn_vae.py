...
class JTNNVAE(nn.Module):
    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG, device="cuda"):
        ...
        self.jtnn    = JTNNEncoder(hidden_size, depthT, nn.Embedding(vocab.size(), hidden_size),device=device)
        self.decoder = JTNNDecoder(vocab, hidden_size, latent_size, nn.Embedding(vocab.size(), hidden_size),device=device)
        self.jtmpn   = JTMPN(hidden_size, depthG, device=device)
        self.mpn     = MPN(hidden_size, depthG, device=device)
    ...
    def rsample(self, z_vecs, W_mean, W_var):
        ...
        epsilon = torch.randn_like(z_mean,device=self.device())
    ...
    def decode(self, x_tree_vecs, x_mol_vecs, prob_decode):
        ...
        jtenc_holder,mess_dict = JTNNEncoder.tensorize_nodes(pred_nodes, scope, device=self.device())
    ...
    def dfs_assemble(
        ...
        if len(cands) > 1:
            ...
            scores = torch.ones(1, 1).float()
        if prob_decode:
            probs = (
                F.softmax(scores.view(1, -1), dim=1).squeeze() + 1e-7
            )  # prevent prob = 0
            cand_idx = torch.multinomial(probs.view(-1), probs.numel())
        else:

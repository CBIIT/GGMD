...
class MPN(nn.Module):
    def __init__(self, hidden_size, depth, device="cuda"):
    ...
    def forward(self, fatoms, fbonds, agraph, bgraph, scope):
        fatoms = Variable(fatoms).to(self.device())
        fbonds = Variable(fbonds).to(self.device())
        agraph = Variable(agraph).to(self.device())
        bgraph = Variable(bgraph).to(self.device())

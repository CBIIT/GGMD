...
class JTNNDecoder(nn.Module):
    def __init__(self, vocab, hidden_size, latent_size, embedding, device="cuda"):
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        pred_hiddens.append(
            torch.zeros(len(mol_batch), self.hidden_size, device=self.device_type)
        )
        ...
        padding = torch.zeros(self.hidden_size, device=self.device_type)
        h = {}
        ...
        for t in range(max_iter):
            ...
            # Clique embedding
            cur_x = torch.LongTensor(cur_x).to(self.device_type)
            ...
            cur_batch = torch.LongTensor(batch_list).to(self.device_type)
            ...
            if len(pred_list) > 0:
                ...
                cur_pred = torch.LongTensor(pred_list).to(self.device_type)
        ...
        stop_hidden = torch.cat([cur_x, cur_o], dim=1)
        stop_hiddens.append(stop_hidden)
        stop_contexts.append(torch.LongTensor(list(range(batch_size))).to(self.device_type))
        ...
        pred_targets = torch.LongTensor(pred_targets).to(self.device_type)
        ...
        stop_targets = torch.Tensor(stop_targets).to(self.device_type)
    ...
    def decode(self, x_tree_vecs, prob_decode):
        ...
        stack = []
        init_hiddens = torch.zeros(1, self.hidden_size, device=self.device_type)
        zero_pad = torch.zeros(1, 1, self.hidden_size, device=self.device_type)
        contexts = torch.LongTensor(1).to(self.device_type).zero_()
